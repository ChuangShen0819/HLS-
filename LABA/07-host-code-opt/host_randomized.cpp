...
auto cpu_begin = std::chrono::high_resolution_clock::now();
  ...
  for(unsigned int n=0; n<numRunsSW; n++)
  {
    ...
    Filter2D_SW(filterCoeffs[filterType], factor, bias, width, height, stride, y_src, y_ref);
    Filter2D_SW(filterCoeffs[filterType], factor, bias, width, height, stride, u_src, u_ref);
    Filter2D_SW(filterCoeffs[filterType], factor, bias, width, height, stride, v_src, v_ref);
  }
